...
uint32 StoreManager::GetAccountCoins(uint32 accountId) const {
	QueryResult result = LoginDatabase.PQuery("SELECT topaze FROM account WHERE id=%u", accountId);
	if (!result)
		return 0;
...
void StoreManager::SetAccountCoins(uint32 accountId, uint32 newCoinsValue) {
	LoginDatabase.PExecute("UPDATE account SET topaze=%u WHERE id=%u", newCoinsValue, accountId);
}
