...
public:
    ...
    void OnLogin(Player* player, bool firstLogin)
    {
        ...
        if (rate != -1 && player->getLevel() != sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL))
        {
            ...
            {
                if (uRate)
                    ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Votre rate xp est de x%u.", uRate);
                else
                    ChatHandler(player->GetSession()).SendSysMessage("|CFF7BBEF7[Custom Rates]|r: Votre rate xp est de  x0. Vous ne gagnerez plus d'exp.");
            }
        }
        ...
        if (rate != -1)
        {
            ...
            {
                if (uRate)
                    ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Votre rate de loot est de x%u.", uRate);
                else
                    ChatHandler(player->GetSession()).SendSysMessage("|CFF7BBEF7[Custom Rates]|r: Votre rate xp est de  x0. Vous ne pourrez plus loot d'objets.");
            }
        }
    }
};
...
public:
    ...
    static bool HandleRateXpCommand(ChatHandler *handler, const char *args)
    {
        ...
        {
            handler->PSendSysMessage("Rate XP désactivé");
        ...
        {
            handler->SendSysMessage("|CFF7BBEF7[Custom Rates]|r: Vous etes deja au niveau maximum.");
            return true;
        }
        ...
        if (!*args)
        {
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Votre rate xp actuel est de x%u.", me->GetCustomXpRate());
            return true;
        }
        ...
        {
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Rate XP invalide, Il doit etre dans l'interval [0,%i].", maxRate);
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        if (player == me)
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Vous avez defini le rate xp a x%u.", uRate);
        else
        {
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Vous avez defini le rate xp de %s a x%u.", handler->GetNameLink(player).c_str(), uRate);
            ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: %s a defini votre rate XP a x%u.", handler->GetNameLink().c_str(), uRate);
        }
    ...
    static bool HandleRateLootCommand(ChatHandler *handler, const char *args)
    {
        ...
        {
            handler->SendSysMessage("|CFF7BBEF7[Custom Rates]|r: Vous etes deja niveau maximum.");
            return true;
        }

        if (sWorld->getIntConfig(CONFIG_CUSTOM_RATE_XP_ENABLED) == 0)
        {
            handler->PSendSysMessage("Rate loot desactive");
            handler->SetSentErrorMessage(true);
        ...
        if (!*args)
        {
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Votre rate loot actuel est de x%u.", me->GetCustomLootRate());
            return true;
        }
        ...
        if (rate < 0 || rate > maxRate)
        {
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Rate loot invalide, Il doit etre dans l'interval [0,%i].", maxRate);
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        // show a message indicating custom XP rate change
        if (player == me)
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Vous avez defini le rate loot a x%u.", uRate);
        else
        {
            handler->PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: Vous avez defini le rate loot de %s a x%u.", handler->GetNameLink(player).c_str(), uRate);
            ChatHandler(player->GetSession()).PSendSysMessage("|CFF7BBEF7[Custom Rates]|r: %s a defini votre rate loot a x%u.", handler->GetNameLink().c_str(), uRate);
        }
